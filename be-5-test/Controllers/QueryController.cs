...
namespace be_5_test.Controllers
{
    public class QueryController : Controller
    {
        ...
        public IActionResult TrasgrePunti()
        {
            ...
            {
                DB.conn.Open();
                var cmd = new SqlCommand("SELECT v.IDAnagrafica, a.Cognome, a.Nome,  sum(v.DecurtamentoPunti) as TotPunti  FROM Verbale as v join dbo.Anagrafica as a on a.IDAnagrafica = v.IDAnagrafica Group by v.IDAnagrafica, a.Cognome, a.Nome", DB.conn);
                var reader = cmd.ExecuteReader();
                if (reader.HasRows)
                {
                    while (reader.Read())
        ...
        public IActionResult FiltroImporto()
        {
            ...
            {
                DB.conn.Open();
                var cmd = new SqlCommand("SELECT a.Cognome, a.Nome, v.IndirizzoViolazione, v.DataViolazione,  v.Importo, v.DecurtamentoPunti  FROM Verbale as v join Anagrafica as a on a.IDAnagrafica = v.IDAnagrafica  WHERE v.Importo > 400", DB.conn);
                var reader = cmd.ExecuteReader();
                if (reader.HasRows)
                {
                    while (reader.Read())
